...
function fieldScore(field, role, rows) {
	const normalizedField = normalized$1(field);
...
function renderReport(input) {
	...
	const list$1 = (items) => items.length > 0 ? items.map((item) => `- ${item}`).join("\n") : "- 暂无";
	return [
		`# ${input.title}`,
		"",
		...
		"",
		"## 主要发现",
		list$1(input.findings),
		"",
		"## 实验与动作",
		list$1(input.experiments),
		"",
		"## 下一周期行动",
		list$1(input.nextActions),
		"",
		"## 口径与限制",
		list$1(input.caveats),
		"",
...
function renderResult(value$1, maxChars) {
	const text$1 = JSON.stringify(value$1, null, 2);
	return [{
		type: "text",
		text: text$1.length > maxChars ? `${text$1.slice(0, maxChars)}\n... result truncated by dsh-growth; use a narrower time window or source ...` : text$1
	}];
}
...
function signalStatus(text$1, pattern) {
	return pattern.test(text$1) ? "ready" : "not-detected";
}
...
function buildGrowthOnboarding(input) {
	...
	const audits = notes;
	const text$1 = notes.map((item) => item.note.content).join("\n");
	...
	const classicDocumentationMethod = (id, name$1, pattern, nextAction) => {
		const projectStatus = signalStatus(text$1, pattern);
		return {
			id,
			name: name$1,
	...
	const methods = [
		...
		methodFromDimension("hadi", "HADI Experiments", "analysis", experimentation),
		{
			...
			name: "RICE / ICE",
			pluginCapability: "analysis",
			projectStatus: signalStatus(text$1, /\bRICE\b|\bICE\b/i),
			evidence: signalStatus(text$1, /\bRICE\b|\bICE\b/i) === "ready" ? ["A priority method is mentioned in the growth notes"] : [],
		...
		},
		{
			...
			name: "Growth Loops",
			pluginCapability: "documentation",
			projectStatus: signalStatus(text$1, /growth loop|增长循环|增长飞轮/i),
			evidence: signalStatus(text$1, /growth loop|增长循环|增长飞轮/i) === "ready" ? ["A loop is mentioned in the growth notes"] : [],
		...
		},
		{
			...
			pluginCapability: "documentation",
			projectStatus: signalStatus(text$1, /external acquisition|directory submission|backlink|外链|目录提交|渠道提交|外部获客/i),
			evidence: signalStatus(text$1, /external acquisition|directory submission|backlink|外链|目录提交|渠道提交|外部获客/i) === "ready" ? ["An external acquisition or directory-submission workflow is mentioned in the growth notes"] : [],
		...
		},
		{
			...
			pluginCapability: "documentation",
			projectStatus: signalStatus(text$1, /AI search|AI readiness|AI discoverability|LLM|AEO|生成式搜索|AI 搜索|AI 可发现性|结构化数据|Product schema|Merchant Center/i),
			evidence: signalStatus(text$1, /AI search|AI readiness|AI discoverability|LLM|AEO|生成式搜索|AI 搜索|AI 可发现性|结构化数据|Product schema|Merchant Center/i) === "ready" ? ["AI search or discoverability readiness is mentioned in the growth notes"] : [],
		...
		classicDocumentationMethod("operating-cadence", "OKR / A3 / OODA / Decision Log", /\bOKR\b|A3 problem|5 Whys|\bOODA\b|决策日志|decision log/i, "使用 growth-strategy-planning，统一目标、问题、行动、决策和复盘记录"),
		{
			...
			name: "WBR / MBR / QBR",
			pluginCapability: "analysis",
			projectStatus: signalStatus(text$1, /\bWBR\b|\bMBR\b|\bQBR\b/i),
			evidence: signalStatus(text$1, /\bWBR\b|\bMBR\b|\bQBR\b/i) === "ready" ? ["An operating review is mentioned in the growth notes"] : [],
...
function text(value$1) {
	return value$1 === void 0 || value$1 === null ? "" : String(value$1).trim();
}
function list(value$1) {
	return Array.isArray(value$1) ? value$1.map(text).filter(Boolean) : [];
}
function consumeGrowthHandoff(input) {
	const warnings = [];
	...
	return {
		...
		status,
		warnings,
		nextActions: status === "ready" ? ["按 primaryMetric、guardrails、window 和数据源运行 growth_review。"] : ["补齐产品结果、主指标、护栏指标和来源后再启动增长复盘。"]
	};
}
function buildMetricContractReview(input) {
	const warnings = [];
	const metrics = input.metrics.flatMap((item, index) => {
		if (typeof item !== "object" || item === null) {
			warnings.push(`metric ${index + 1} 不是对象。`);
			return [];
		}
		const record = item;
		const name$1 = text(record.name);
		...
		const currency = text(record.currency) || void 0;
		const status = name$1 && event && window && timezone ? "ready" : "partial";
		if (status === "partial") warnings.push(`${name$1 || `metric ${index + 1}`} 缺少 name、event、window 或 timezone。`);
		return [{
			name: name$1,
			event,
			window,
	...
	return {
		...
		metrics,
		warnings,
		nextActions: warnings.length > 0 ? [`补齐指标定义；默认币种为 ${input.currency}，默认时区为 ${input.timezone}。`] : ["将指标契约绑定到事件数据和 growth_review 的 lineage。"]
	};
}
...
const artifactDomain = "dsh-growth";
function normalized(value$1) {
	if (Array.isArray(value$1)) return value$1.map(normalized);
	if (value$1 && typeof value$1 === "object") return Object.fromEntries(Object.entries(value$1).filter(([key]) => ![
		"artifactId",
		"generatedAt",
		"capturedAt",
		"markdown",
		"warnings",
		"nextActions"
	].includes(key)).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, normalized(item)]));
	return value$1;
}
function contentHash(value$1) {
	const raw = typeof value$1 === "string" ? value$1 : JSON.stringify(normalized(value$1));
	return createHash("sha256").update(raw).digest("hex").slice(0, 16);
}
function slug(value$1) {
	return value$1.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "artifact";
}
function createArtifactId(value$1) {
	return `${artifactDomain}-${slug(typeof value$1.artifactType === "string" ? value$1.artifactType : "artifact")}-${contentHash(value$1).slice(0, 12)}`;
}
...
function attachArtifactMetadata(value$1, options = {}) {
	if (!value$1 || typeof value$1 !== "object" || Array.isArray(value$1)) return value$1;
	const record = value$1;
	if (typeof record.artifactType !== "string" || !record.artifactType.trim()) return value$1;
	const generatedAt = typeof record.generatedAt === "string" && record.generatedAt ? record.generatedAt : (/* @__PURE__ */ new Date()).toISOString();
	const metadata = {
		schemaVersion: typeof record.schemaVersion === "string" ? record.schemaVersion : "1.0",
		artifactId: typeof record.artifactId === "string" && record.artifactId ? record.artifactId : createArtifactId({
			...record,
			generatedAt: void 0
		}),
		generatedAt,
	...
	if (options.staleAfterDays !== void 0) metadata.staleAfter = typeof record.staleAfter === "string" ? record.staleAfter : addDays(generatedAt, options.staleAfterDays);
...
function reviewArtifact(value$1, expectedType) {
	const issues = [];
	const warnings = [];
	if (!value$1 || typeof value$1 !== "object" || Array.isArray(value$1)) return {
		status: "blocked",
		issues: ["工件必须是 JSON object。"],
		warnings,
		nextActions: ["传入插件实际生成的 JSON 工件，而不是 Markdown 展示文本。"]
	};
	const record = value$1;
	const artifactType = typeof record.artifactType === "string" ? record.artifactType : void 0;
	const artifactId = typeof record.artifactId === "string" ? record.artifactId : void 0;
	const sourceHash = typeof record.sourceHash === "string" ? record.sourceHash : void 0;
	const artifactContentHash = typeof record.contentHash === "string" ? record.contentHash : void 0;
	const staleAfter = typeof record.staleAfter === "string" ? record.staleAfter : void 0;
	...
	return {
		...
		staleAfter,
		nextActions: status === "ready" ? ["可以交给目标插件继续处理，并保留 artifactId 作为回溯入口。"] : ["补齐缺失的工件元数据后再进入下一阶段。"]
	};
}
async function appendArtifactAudit(fs, root, event, signal) {
	const path = join(root, ".dsh-growth-audit.jsonl");
	const target = await fs.resolve(path, { signal });
	const info = await fs.stat(target, signal);
	const next = `${info?.type === "file" ? await fs.readText(target, signal) : ""}${JSON.stringify({
		...event,
		plugin: artifactDomain,
		recordedAt: (/* @__PURE__ */ new Date()).toISOString()
	})}\n`;
	await fs.writeText(target, next, info ? {
		kind: "replaceIfVersion",
		version: info.version
	} : { kind: "createIfAbsent" }, signal);
	return {
		path,
		entries: next.trim() ? next.trim().split(/\r?\n/).length : 0
	};
}
...
function wrapResult(value$1, options = {}) {
	const warnings = typeof value$1 === "object" && value$1 !== null && "warnings" in value$1 && Array.isArray(value$1.warnings) ? value$1.warnings.filter((warning) => typeof warning === "string") : [];
	return resultEnvelope({
		data: attachArtifactMetadata(value$1, { staleAfterDays: 30 }),
		warnings,
		assumptions: options.assumptions,
		lineage: options.lineage,
		nextActions: options.nextActions
...
function registerGrowthTools(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: { handoffJson: {
			type: "string",
			required: true,
			description: "JSON returned by product_growth_handoff, including its result envelope or data object."
		} },
		output: growthOutput(config.maxResultChars),
		async execute(args) {
			let parsed;
			try {
				parsed = JSON.parse(args.handoffJson);
			} catch (error) {
				throw new Error(`handoffJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
			}
			...
			const result = consumeGrowthHandoff(data);
	...
	ctx.tools.register(defineTool({
		...
		parameters: { metricsJson: {
			type: "string",
			required: true,
			description: "JSON array of metric definitions."
		} },
		output: growthOutput(config.maxResultChars),
		async execute(args) {
			let parsed;
			try {
				parsed = JSON.parse(args.metricsJson);
			} catch (error) {
				throw new Error(`metricsJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
			}
			const metrics = typeof parsed === "object" && parsed !== null && !Array.isArray(parsed) && "data" in parsed ? parsed.data : parsed;
			if (!Array.isArray(metrics)) throw new Error("metricsJson must contain an array.");
			const result = buildMetricContractReview({
				metrics,
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			measurementJson: {
				type: "string",
				required: true,
				description: "One measurement plan, an array, or a result envelope containing measurement plans."
			},
			metricsJson: {
				type: "string",
				required: true,
				description: "JSON array of {contentId, channel, metric, value, baseline?, window, source?} observations."
			}
		},
		output: growthOutput(config.maxResultChars),
		async execute(args) {
			const unwrap = (value$1) => typeof value$1 === "object" && value$1 !== null && "data" in value$1 ? value$1.data : value$1;
			let measurementValue;
			let metricsValue;
			try {
				measurementValue = unwrap(JSON.parse(args.measurementJson));
				metricsValue = JSON.parse(args.metricsJson);
			} catch (error) {
				throw new Error(`measurementJson and metricsJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
			}
			...
			const observations = metricsValue.filter((item) => typeof item === "object" && item !== null && !Array.isArray(item));
			const results = plans.filter((item) => typeof item === "object" && item !== null && !Array.isArray(item)).map((plan) => {
				...
				return {
					...
					channel,
					targetMetric,
					status: matched.length === 0 ? "unmeasured" : comparable.length === matched.length ? "measured" : "partial",
					observations: matched.length,
					comparableObservations: comparable.length,
					deltas: comparable.map((row) => ({
						window: row.window,
						delta: Number(row.value) - Number(row.baseline),
						source: row.source
					}))
				};
			});
			...
			return wrapResult({
				artifactType: "growth-attribution-review",
				generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			artifactJson: {
				type: "string",
				required: true,
				description: "JSON returned by a plugin tool."
			},
			expectedType: {
				type: "string",
				description: "Optional expected artifactType."
			}
		},
		output: growthOutput(config.maxResultChars),
		async execute(args) {
			let value$1;
			try {
				value$1 = JSON.parse(args.artifactJson);
			} catch (error) {
				throw new Error(`artifactJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
			}
			const review = reviewArtifact(typeof value$1 === "object" && value$1 !== null && "data" in value$1 ? value$1.data : value$1, args.expectedType?.trim() || void 0);
			return wrapResult({
				artifactType: "growth-artifact-review",
				generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
				...review
			}, { nextActions: review.nextActions });
		}
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			let audit;
			try {
				audit = await appendArtifactAudit(fs, config.defaultRoot, {
					action: "apply",
					path: args.path,
					beforeHash: contentHash(current),
					afterHash: contentHash(args.content),
					approved: true
				}, exec.signal);
			} catch (error) {
				audit = {
