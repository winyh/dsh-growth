...
function statusFor(rowCount, selectedFields, invalidDateRows, invalidNumberRows) {
...
function amount(rows, typeField, amountField, type, mode) {
	return rows.filter((row) => stringValue(row, typeField)?.toLowerCase() === type).reduce((sum, row) => {
		const value$1 = numberValue(row, amountField) ?? 0;
		return sum + (mode === "absolute" ? Math.abs(value$1) : value$1);
	}, 0);
}
...
function analyzeEconomics(source, rows, options) {
	...
	periods.forEach((period, index) => {
		...
		const movementDelta = amountMode === "signed" ? newMrr + expansionMrr + reactivationMrr + contractionMrr + churnedMrr : newMrr + expansionMrr + reactivationMrr - contractionMrr - churnedMrr;
		const endingMrr$1 = explicitEnding ?? (beginningMrr === null ? null : movementSource === "snapshot" ? beginningMrr : beginningMrr + movementDelta);
		const activeCustomers = currentRows.map((row) => numberValue(row, "active_customers")).find((value$1) => value$1 !== void 0) ?? new Set(currentRows.map((row) => stringValue(row, options.customerField)).filter((value$1) => Boolean(value$1))).size;
		...
		const arpa = activeCustomers > 0 && endingMrr$1 !== null ? endingMrr$1 / activeCustomers : null;
		...
		const nrr = safeDivide((beginningMrr === null || movementSource === "snapshot" ? null : amountMode === "signed" ? beginningMrr + expansionMrr + reactivationMrr + contractionMrr + churnedMrr : beginningMrr + expansionMrr + reactivationMrr - contractionMrr - churnedMrr) ?? 0, beginningMrr ?? 0);
...
function eventKey(value$1) {
	return value$1.toLowerCase().trim().replace(/[^a-z0-9]+/g, "_");
}
...
function registerGrowthTools(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const profiles = [];
			const lineage = [];
			emitAnalysisStarted(ctx, "review", [
				args.eventPath,
				args.economicsPath,
				args.notePath
			].filter((path) => Boolean(path)), args.goal);
			let funnel;
			let cohort;
			let economics;
			let noteAudit;
			if (args.eventPath?.trim()) {
				...
				const profile = growthData?.profileDataset(args.eventPath, dataset.rows, {
					userField: args.userField,
					eventField: args.eventField,
					timeField: args.timeField
				}) ?? profileDataset(args.eventPath, dataset.rows, {
					userField: args.userField,
					eventField: args.eventField,
					timeField: args.timeField
				});
			...
			if (args.economicsPath?.trim()) {
				...
				const profile = growthData?.profileDataset(args.economicsPath, dataset.rows, {
					periodField: args.periodField,
					typeField: args.typeField,
					amountField: args.amountField
				}) ?? profileDataset(args.economicsPath, dataset.rows, {
					periodField: args.periodField,
					typeField: args.typeField,
					amountField: args.amountField
				});
